...
function App(): JSX.Element {
  ...
  // Ref so SSE message handlers (captured in stale closures) always see the latest API instance
  ...
  useEffect(() => {
    if (excalidrawAPI) {
      ...
      // Ensure SSE is connected for real-time updates
      if (!isConnected) {
        connectSSE()
      }
    }
  ...
  const handleSSEMessage = async (data: StreamMessage): Promise<void> => {
    ...
    try {
        ...
        case 'mermaid_convert':
          ...
          if (data.mermaidDiagram) {
            try {
              ...
              console.error('Error converting Mermaid diagram from SSE:', error)
            }
          }
          break
      ...
      console.error('Error processing SSE message:', error, data)
    }
  }
