...
app.post('/api/elements/from-mermaid', (req: Request, res: Response) => {
  try {
    ...
    // Broadcast to all SSE clients to process the Mermaid diagram
    broadcast({
...
app.post('/api/elements/sync', (req: Request, res: Response) => {
  try {
    ...
    // 3. Broadcast sync event to all SSE clients
    broadcast({
...
// Image export: request (MCP -> Express -> SSE -> Frontend)
...
app.post('/api/export/image/result', (req: Request, res: Response) => {
  try {
    ...
    if (error) {
      // Don't reject on error — another SSE client may still succeed.
...
// Viewport control: request (MCP -> Express -> SSE -> Frontend)
...
server.listen(PORT, HOST, () => {
  ...
  logger.info(`SSE stream available on http://${HOST}:${PORT}/api/stream`);
});
