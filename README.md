...
- Canvas server: web UI + REST API + SSE updates (default `http://localhost:3000`)
...
| **Live canvas UI** | Rendered inline in chat | Standalone Excalidraw app synced via SSE |
...
Canvas server:
```bash
docker run -d -p 3000:3000 --name mcp-excalidraw-canvas ramkumarar/mcp-excalidraw-canvas:latest
```

MCP server (stdio) is typically launched by your MCP client (Claude Desktop/Cursor/etc.). If you want a local container for it, use the image `ramkumarar/mcp-excalidraw:latest` and set `EXPRESS_SERVER_URL` to point at the canvas.
  ...
  "mcpServers": {
    "excalidraw": {
      ...
      "args": [
        ...
        "ramkumarar/mcp-excalidraw:latest"
      ]
    }
  }
}
...
```json
{
  ...
  "mcp": {
    "excalidraw": {
      "type": "local",
      "command": ["docker", "run", "-i", "--rm", "-e", "EXPRESS_SERVER_URL=http://host.docker.internal:3000", "-e", "ENABLE_CANVAS_SYNC=true", "ramkumarar/mcp-excalidraw:latest"],
      "enabled": true
    }
  }
}
...
- **Canvas server**: Must be running before the MCP server connects. Start it with `npm run canvas` (local) or `docker run -d -p 3000:3000 ramkumarar/mcp-excalidraw-canvas:latest` (Docker).
